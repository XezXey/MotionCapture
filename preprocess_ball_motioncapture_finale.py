...
def load_config_file(folder_name, idx):
  ...
  for each_node in nodes_extrinsic:
    if each_node == "w" or each_node == "h":
      ...
      translation = extrinsic.getNode(each_node).mat()[:-1, -1].reshape(-1, 1)
  ...
  for idx in range(len(trajectory_split)):
    ...
    if len(ground_annotated[idx]) <= 2:
      ...
      if args.scale:
        ...
        trajectory_split[idx]['ball_plane_x'] *= (max_x/8)
        trajectory_split[idx]['ball_plane_y'] *= (max_y/4)
        trajectory_split[idx]['ball_plane_z'] *= (max_z/8)
